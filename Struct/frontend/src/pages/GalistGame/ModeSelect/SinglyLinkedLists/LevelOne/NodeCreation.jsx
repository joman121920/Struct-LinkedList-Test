...
import { playHitSound, playClaimSound, playErrorSound, playFirstClickSound, stopNodeCreationBgMusic } from "../../../Sounds.jsx";
...
function MainGameComponent() {
  ...
  const loadExercise = useCallback((key = "level_1") => {
    ...
    setSuckedCircles([]);
    setCurrentEntryOrder([]);
    // Floating circles will be regenerated by the useEffect when currentExercise/exerciseKey change
    ...
    if (hitSoundsPlayedRef) hitSoundsPlayedRef.current.clear(); // Clear hit sounds tracking
    setOriginalSubmission(null);
    setShowValidationResult(false);
    setValidationResult(null);
  ...
  const handleRetry = () => {
    ...
    // Force clear floating circles first to trigger regeneration
    setFloatingCircles([]);
    
    // Clear all other states to ensure clean reset
    setCircles([]);
    setConnections([]);
    setSuckingCircles([]);
    setSuckedCircles([]);
    ...
    if (entryOrderRef) entryOrderRef.current = [];
    if (suckedCirclesRef) suckedCirclesRef.current = [];
    if (hitSoundsPlayedRef) hitSoundsPlayedRef.current.clear();
    
    // Force regeneration by temporarily setting to null then back to level_1
    setCurrentExercise(null);
    setExerciseKey("");
    
    // Use setTimeout to ensure state updates complete before loading exercise
    setTimeout(() => {
      loadExercise("level_1");
    }, 50);
  };
            ...
            for (let i = 0; i < currentFloatingCircles.length; i++) {
              ...
              if (centerDistance < collisionThreshold) {
                ...
                if (isValidHit) {
                  ...
                  // Create unique collision ID to prevent duplicate sounds
                  const collisionId = `${updatedCircle.id}-${floatingCircle.id}`;
                  ...
                  setSquareNode(prev => {
                    ...
                    if (floatingCircle.type === 'value') {
                      ...
                      if (!currentValue) {
                        // Empty slot - check if shooting correct value
                        if (newValue === correctValue) {
                          newSquareNode.value = newValue;
                          if (!hitSoundsPlayedRef.current.has(collisionId)) {
                            playHitSound(); // Play hit sound for correct value
                            hitSoundsPlayedRef.current.add(collisionId);
                          }
                        } else {
                          // Shooting wrong value into empty slot
                          newSquareNode.value = newValue;
                          if (!hitSoundsPlayedRef.current.has(collisionId)) {
                            playErrorSound();
                            hitSoundsPlayedRef.current.add(collisionId);
                        ...
                        // Have correct value, shooting incorrect - DON'T override and play error
                        if (!hitSoundsPlayedRef.current.has(collisionId)) {
                          playErrorSound();
                          hitSoundsPlayedRef.current.add(collisionId);
                        }
                        ...
                        // Have incorrect value, shooting another incorrect - OVERRIDE and play error
                        newSquareNode.value = newValue;
                        if (!hitSoundsPlayedRef.current.has(collisionId)) {
                          playErrorSound();
                          hitSoundsPlayedRef.current.add(collisionId);
                      ...
                      if (!currentAddress) {
                        // Empty slot - check if shooting correct address
                        if (newAddress === correctAddress) {
                          newSquareNode.address = newAddress;
                          if (!hitSoundsPlayedRef.current.has(collisionId)) {
                            playHitSound(); // Play hit sound for correct address
                            hitSoundsPlayedRef.current.add(collisionId);
                          }
                        } else {
                          // Shooting wrong address into empty slot
                          newSquareNode.address = newAddress;
                          if (!hitSoundsPlayedRef.current.has(collisionId)) {
                            playErrorSound();
                            hitSoundsPlayedRef.current.add(collisionId);
                        ...
                        // Have correct address, shooting incorrect - DON'T override and play error
                        if (!hitSoundsPlayedRef.current.has(collisionId)) {
                          playErrorSound();
                          hitSoundsPlayedRef.current.add(collisionId);
                        }
                        ...
                        // Have incorrect address, shooting another incorrect - OVERRIDE and play error
                        newSquareNode.address = newAddress;
                        if (!hitSoundsPlayedRef.current.has(collisionId)) {
                          playErrorSound();
                          hitSoundsPlayedRef.current.add(collisionId);
      ...
      {circles.map((circle) => (
        <div
          ...
          style={{
            ...
            backgroundColor: circle.isBullet ? '#ff00b3ff' : '#d3d3d3',
        ...
        <div className={styles.completionOverlay}>
          <div className={styles.completionPopup}>
            <div className={styles.completionContent}>
              ...
              <button 
                onClick={() => { handleContinueToNextLevel(); playClaimSound(); }}
        ...
        <div className={styles.completionOverlay}>
          <div className={styles.completionPopup}>
            <div className={styles.completionContent}>
              <h2>{completionMessage}</h2>
              <button 
                onClick={() => {handleGoBack(); stopNodeCreationBgMusic();}}
                className={styles.completionButton}
              >
                Go back
              </button>
