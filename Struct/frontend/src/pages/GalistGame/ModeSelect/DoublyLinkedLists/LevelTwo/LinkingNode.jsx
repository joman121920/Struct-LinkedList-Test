 ...
 const handleCircleClick = useCallback((circleId, e) => {
  ...
  if (exerciseData && exerciseData.expectedStructure && exerciseData.expectedStructure.length > 0) {
    ...
    const initialCircle = {
      ...
      prevAddress: NULL_POINTER, // Initialize with null
      nextAddress: NULL_POINTER, // Initialize with null
      clickCount: 0,
      deletionReady: false,
      isInitial: true,
      isLaunched: true
                  ...
                  <React.Fragment key={node.address}>
                    <td className={styles.expectedBarCell}>
                      <div className={styles.expectedBarCircle}>
                        <span className={styles.expectedBarAddress}>
                              ...
                              >
                                <td className={styles.userCell}>
                                  {userNode ? (
                                    ...
                                    >
                                      <span className={styles.userNodePointer}>
                                        {userNode.prevAddress ?? NULL_POINTER}
                                      </span>
                                      <span className={styles.userNodeValue}>
                                        {userNode.value}
                                      </span>
                                      <span className={styles.userNodePointer}>
                                        {userNode.nextAddress ?? NULL_POINTER}
                                      </span>
                                    </div>
                                  ) : (
                                    <div
    ...
    >
      ...
      <div style={{
        ...
        {bulletOptions.map((bullet) => (
          ...
          >
            <span style={{ fontSize: '10px', color: '#555', fontWeight: 600, lineHeight: 1, marginBottom: '3px' }}>
              {bullet.prevAddress ?? NULL_POINTER}
            </span>
            <span style={{ fontSize: '18px', fontWeight: 'bold', lineHeight: 1, margin: '5px 0' }}>
              {bullet.value}
            </span>
            {/* REMOVED bullet.address line - not needed for doubly linked list display */}
            <span style={{ fontSize: '10px', color: '#555', fontWeight: 600, lineHeight: 1, marginTop: '3px' }}>
