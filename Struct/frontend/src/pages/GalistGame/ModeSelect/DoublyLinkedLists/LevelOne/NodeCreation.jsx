...
function MainGameComponent() {
  ...
  const [bulletsRemaining, setBulletsRemaining] = useState(20); // Start with 10 bullets per exercise
  const [showFailedModal, setShowFailedModal] = useState(false);
  const [maxBulletsPerExercise] = useState(20); // Constant bullets per exercise
  ...
  const generateFloatingCircles = () => {
    ...
    const circles = circleData.map((circleInfo) => {
      ...
      return {
        id: circleInfo.id,
        type: circleInfo.type, // 'value', 'address', or 'prevAddress'
  ...
  useEffect(() => {
    if (squareNode.prevAddress && squareNode.value && squareNode.address && currentExercise) {
  ...
  const loadExercise = useCallback((key = "level_1") => {
    ...
    setSquareNode({ prevAddress: "",value: "", address: "" });
    ...
    setExpectedOutput({
      prevAddress: exercise.expectedOutput.prevAddress,
              ...
              if (centerDistance < collisionThreshold) {
                ...
                if (isValidHit) {
                  ...
                  setSquareNode(prev => {
                    ...
                    } else if (floatingCircle.type === 'prevAddress') {
                      // ADD PREV ADDRESS LOGIC
                      const correctPrevAddress = expectedOutput.prevAddress;
                      const currentPrevAddress = prev.prevAddress;
                      const newPrevAddress = floatingCircle.content;
                      
                      if (!currentPrevAddress) {
                        newSquareNode.prevAddress = newPrevAddress;
                      } else if (currentPrevAddress === correctPrevAddress && newPrevAddress !== correctPrevAddress) {
                        // Don't override correct prev address
                      } else if (currentPrevAddress !== correctPrevAddress && newPrevAddress === correctPrevAddress) {
                        newSquareNode.prevAddress = newPrevAddress;
                      } else if (currentPrevAddress !== correctPrevAddress && newPrevAddress !== correctPrevAddress) {
                        newSquareNode.prevAddress = newPrevAddress;
  ...
  }, [expectedOutput.value, expectedOutput.address, expectedOutput.prevAddress]); // Include expectedOutput dependencies for smart overriding logic
      ...
      {floatingCircles.map(circle => (
         <div
          ...
          className={`${styles.floatingCircle} ${
            circle.type === 'value' 
              ? styles.valueCircle 
              : circle.type === 'prevAddress'
                ? styles.prevAddressCircle
                : styles.addressCircle
          }`}
          style={{
            left: `${circle.x}px`,
            top: `${circle.y}px`,
