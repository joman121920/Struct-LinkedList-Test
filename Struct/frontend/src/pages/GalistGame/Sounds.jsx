...
const initializeAudio = () => {
  try {
    ...
    // Preload deletion background music
    audioInstances.deletionBg = new Audio("/sounds/deletion_bgm.mp3");
    audioInstances.deletionBg.preload = "auto";
    audioInstances.deletionBg.volume = 0.14; // Lower volume for deletion music
    audioInstances.deletionBg.loop = true; // Loop the deletion music
...
// Start playing deletion background music (looped)
export const playDeletionBgMusic = () => {
  if (!musicEnabled) return;

  try {
    if (!audioInstances.deletionBg) {
      audioInstances.deletionBg = new Audio("/sounds/deletion_bgm.mp3");
      audioInstances.deletionBg.volume = 0.14;
      audioInstances.deletionBg.loop = true;
    }

    // Prevent double playback
    if (!audioInstances.deletionBg.paused) {
      return;
    }

    audioInstances.deletionBg.volume = 0.14;
    audioInstances.deletionBg.currentTime = 0;
    audioInstances.deletionBg.loop = true;

    audioInstances.deletionBg
      .play()
      .then(() => {})
      .catch((error) => {
        console.warn("Could not play deletion background music:", error);
      });
  } catch (error) {
    console.warn("Error starting deletion background music:", error);
  }
};

// Stop deletion background music
export const stopDeletionBgMusic = () => {
  try {
    if (audioInstances.deletionBg) {
      audioInstances.deletionBg.pause();
      audioInstances.deletionBg.currentTime = 0;
    }
  } catch (error) {
    console.warn("Error stopping deletion background music:", error);
  }
};
